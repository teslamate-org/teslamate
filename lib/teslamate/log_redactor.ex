defmodule TeslaMate.LogRedactor do
  ...
  @key_value_regex ~r/(?i)((?<![a-z0-9_])["']?(?:access|access_token|client_secret|database_pass|encryption_key|mqtt_password|password|pgpassword|refresh|refresh_token|secret_key_base|teslamate_operations_password|token)["']?\s*+(?>=>|:|=)\s*+["']?)(?!\[REDACTED(?:_JWT)?\])([^"',}\]&\]\r\n]+)/
  ...
  def redact(text) when is_binary(text) do
    ...
    |> replace(@authorization_regex, "\\1[REDACTED]")
    |> replace(@bearer_regex, "\\1[REDACTED]")
    |> replace(@key_value_regex, "\\1[REDACTED]")
