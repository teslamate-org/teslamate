...
- Disable anonymous logins to Grafana (when using the `teslamate/grafana` Docker image)
    ...
    - To allow anynomous logins set the environmet variable of the Grafana image `GF_AUTH_ANONYMOUS_ENABLED` to `true` (use only if your Grafana instance is not exposed to the internet!)
