...
The work already paid off: it exposed a crash in the update-cancel path (#5656) and a crash loop after an offline period when the car reports an outdated timestamp (#5684), both fixed in this release, and it surfaced two quirks for a later fix — a charge sample without charger power is stored as 0 kW (#5699), and the published state start time can jump backwards after charging, updating or driving (#5693).
