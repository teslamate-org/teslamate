defmodule TeslaMate.Repo.Migrations.AddAndCalculateElevationChanges do
  ...
  def up do
    ...
    WITH elevation_changes AS (
      SELECT
        drive_id,
        COALESCE(NULLIF(LEAST(SUM(CASE WHEN elevation_diff > 0 THEN elevation_diff ELSE 0 END), 32767), 32767), 0) as ascent,
        COALESCE(NULLIF(LEAST(SUM(CASE WHEN elevation_diff < 0 THEN ABS(elevation_diff) ELSE 0 END), 32767), 32767), 0) as descent
      FROM (
        SELECT
          drive_id,
