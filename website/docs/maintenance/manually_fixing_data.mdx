...
**NOTE:** Maintaining the efficiency of database operations is imperative for any application relying on relational databases. One of the tools provided by PostgreSQL to help maintain performance is the REINDEX command. It is recommended that the database be reindexed periodically to keep it up to date, at least once a month or more frequently if you use the car intensively.
...
2. Run the followin script (copy and paste)
