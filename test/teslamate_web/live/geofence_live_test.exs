defmodule TeslaMateWeb.GeoFenceLiveTest do
  ...
  describe "Edit" do
    test "validates changes when editing of a geo-fence", %{conn: conn} do
      ...
      for kind <- ["name", "radius"] do
        ...
        assert error_html ==
                 "<span class=\"help is-danger pl-15\" phx-feedback-for=\"#{id}_#{kind}\">can't be blank</span>"
      end
    end
  ...
  describe "New" do
    ...
    test "validates changes when creating a new geo-fence", %{conn: conn} do
      ...
      assert ["GBP"] = html |> Floki.find("#geo_fence_currency_code") |> Floki.attribute("value")
