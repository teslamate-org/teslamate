defmodule TeslaMateWeb.SettingsLiveTest do
  ...
  test "shows km and C by default", %{conn: conn} do
    ...
    assert html =~
             ~r(<select id="settings_unit_of_length" .+><option value="km" selected="selected">km</option><option value="mi">mi</option></select>)

    assert html =~
             ~r(<select id="settings_unit_of_temperature" .+><option value="C" selected="selected">°C</option><option value="F">°F</option></select>)
  end
  ...
  test "shows :ideal by default", %{conn: conn} do
    ...
    assert html =~
             ~r(<select id="settings_preferred_range" .+><option value="ideal" selected="selected">ideal</option><option value="rated">rated</option></select>)
  end
  ...
  test "shows 21 and 15 minutes by default", %{conn: conn} do
    ...
    assert html =~
             ~r(<select id="settings_suspend_min" .+>.*<option value="21" selected="selected">21 min</option>.*</select>)

    assert html =~
             ~r(<select id="settings_suspend_after_idle_min" .+>.*<option value="15" selected="selected">15 min</option>.*</select>)
  end
