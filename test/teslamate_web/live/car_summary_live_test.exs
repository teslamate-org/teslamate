defmodule TeslaMateWeb.CarLive.SummaryTest do
  ...
  describe "tags" do
    ...
    defp software_update_icon(conn, software_update) do
      ...
      |> Floki.parse_document!()
      |> Floki.find(".icons .icon")
      |> Enum.find(&match?({"span", _, [{"span", [{"class", "mdi mdi-gift-outline"} | _], _}]}, &1))
    end
    ...
    test "shows 'Installing' and colors the icon while installing", %{conn: conn} do
      ...
      assert {"span", [{"class", "mdi mdi-gift-outline"}, {"style", "color: #4caf50;"}], _} = inner
    end
    ...
    test "shows 'available' tooltip and colors the icon when update is scheduled", %{conn: conn} do
      ...
      assert {"span", [{"class", "mdi mdi-gift-outline"}, {"style", "color: #e8a33d;"}], _} = inner
    end
