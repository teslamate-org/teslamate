defmodule TeslaMate.LogCarDisplayOrderTest do
  ...
  test "renumbers all cars 1..N after the first move" do
    ...
    assert {:ok, [b.id, a.id, c.id, d.id, e.id, f.id]} == Log.move_car(a.id, :right)

    assert priorities_by_id() ==
             [{a.id, 2}, {b.id, 1}, {c.id, 3}, {d.id, 4}, {e.id, 5}, {f.id, 6}]
  end

  test "orders equal priorities by id and swaps with the left neighbour" do
    [a, b, c, d] = cars_with_priorities([1, 1, 1, 1])

    assert {:ok, [a.id, c.id, b.id, d.id]} == Log.move_car(c.id, :left)
  ...
  test "changes nothing when a car is already at either end" do
    ...
    assert {:ok, [a.id, b.id, c.id]} == Log.move_car(a.id, :left)
    assert {:ok, [a.id, b.id, c.id]} == Log.move_car(c.id, :right)
  ...
  test "returns an error for an unknown car" do
    ...
    assert {:error, :not_found} == Log.move_car(-1, :left)
  end
end
